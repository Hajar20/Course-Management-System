...
from .models import Users,classSubject,course,TD,TP,correction_TD_TP,Todo,JoinClass
import datetime 
...
def sendingHW(request) :
    if request.method == 'POST' :
        ...
        sendHW = Todo(title=desc,creationDateTodo=dateCreation,TodoTFile=fileHW,TodoSFile='',course=cours)
        sendHW.save()
        classe =  classSubject.objects.filter(userId=user)
        classeS = classSubject.objects.filter(userId=user)
...
def addingCorr(request) :
    if request.method == 'POST' :
        ...
        classeName = request.POST.get('course')
        cours = classSubject.objects.get(titleClass = classeName)
        Corrfile = correction_TD_TP(title=desc,creationDateTdTp=dateCreation,corrFile=fileCorr,course=cours)
        Corrfile.save()
        classe =  classSubject.objects.filter(userId=user)
        classeS = classSubject.objects.filter(userId=user)
        return  render(request,'Teacher/allCourses.html',{'user' :user ,'classe' : classe,'classeS' : classeS})
...
def addingTP(request) :
    if request.method == 'POST' :
        ...
        classeName = request.POST.get('course')
        cours = classSubject.objects.get(titleClass = classeName)
        tpfile = TP(titleTp=desc,creationDateTp=dateCreation,tpFile=fileTP,course=cours)
        tpfile.save()
        classe =  classSubject.objects.filter(userId=user)
        classeS = classSubject.objects.filter(userId=user)
        return  render(request,'Teacher/allCourses.html',{'user' :user ,'classe' : classe,'classeS' : classeS}) 
...
def addingTD(request):
    if request.method == 'POST' :
        ...
        classeName = request.POST.get('course')
        cours = classSubject.objects.get(titleClass = classeName)
        tdfile = TD(titleTd=desc,creationDateTd=dateCreation,tdFile=fileTD,course=cours)
        tdfile.save()
        classe =  classSubject.objects.filter(userId=user)
        classeS = classSubject.objects.filter(userId=user)
        return  render(request,'Teacher/allCourses.html',{'user' :user ,'classe' : classe,'classeS' : classeS}) 
...
def addingCourse(request) :
    if request.method == 'POST' :
        ...
        classeName = request.POST.get('course')
        cours = classSubject.objects.get(titleClass = classeName)
        coursefile = course(desc=desc,creationDateCourse=dateCreation,courseFile=fileC,course=cours)
        coursefile.save()
        classe =  classSubject.objects.filter(userId=user)
        classeS =  classSubject.objects.filter(userId=user)
        return  render(request,'Teacher/allCourses.html',{'user' :user ,'classe' : classe,'classeS' : classeS}) 
...
def addCourse(request) :
      if request.method == 'POST' :
        ...
        user = Users.objects.get(id=id)
        c = classSubject(titleClass=titleClass,level=levelClass,codeClasse=codeClass,creationDateClasse=dateOfCreation,userId=user)
        c.save()
        classe = classSubject.objects.filter(userId=user)
        classeS = classSubject.objects.filter(userId=user)
        return render(request,'Teacher/allCourses.html',{'classe' : classe , 'user' : user ,'classeS' : classeS})
...
def updatingData(request) :
    if request.method == 'POST' :
        ...
        if bool(request.FILES.get('fileImgUpdate', False)) == True:
             ...
             user = Users(id=id,nom = nom, prenom =prenom,email=email,password=password,picture=picture,role=role)
             user.save()
             classeS =  classSubject.objects.filter(userId=user)
